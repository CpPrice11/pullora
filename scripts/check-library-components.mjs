...
try {
  ...
  assert.doesNotMatch(cropDialogMarkup, /type="range"/)
  assert.match(cropDialogMarkup, /art-crop-canvas/)
  ...
  const artCropPointerMoveSource = artCropDialogSource.slice(
    ...
    artCropDialogSource.indexOf('const finishInteraction'),
  )
  const modalStylesSource = readFileSync('src/components/Modal/Modal.css', 'utf8')
  const installationPathModalSource = readFileSync('src/components/Modal/InstallationPathModal.tsx', 'utf8')
  const folderManagerSource = readFileSync('src/features/library/components/FolderManager.tsx', 'utf8')
  const uninstallConfirmSource = readFileSync('src/features/library/components/UninstallConfirmModal.tsx', 'utf8')
  ...
  assert.doesNotMatch(pageStylesSource, /:root\[data-theme='light'\] \.(?:confirm-primary-btn|hero-primary-btn)\s*\{[^}]*#111827/s)
  assert.doesNotMatch(searchComponentsStyles, /:root\[data-theme='light'\] \.release-action-primary\s*\{[^}]*#111827/s)
  ...
  assert.doesNotMatch(artCropDialogSource, /type="range"/)
  assert.match(artCropDialogSource, /cropFromFrame[\s\S]*?zoom: baseWidth \/ frame\.width/)
  assert.match(artCropDialogSource, /className=\{`art-crop-handle art-crop-handle--\$\{corner\}`\}/)
  ...
  assert.doesNotMatch(libraryPageSource, /page\.classList\.remove\('library-density-normal'/)
  assert.match(artCropDialogSource, /<img[\s\S]*?naturalWidth[\s\S]*?naturalHeight/)
  assert.match(modalStylesSource, /\.art-crop-preview\s*\{[^}]*0 0 0 9999px rgba\(2, 7, 12, 0\.62\)/s)
  assert.match(modalStylesSource, /\.art-crop-handle--top-left[\s\S]*?\.art-crop-handle--bottom-right/)
  ...
  assert.equal((folderManager.match(/class="primary-btn"/g) ?? []).length, 1)
  assert.match(folderManagerSource, /className="secondary-btn"[\s\S]*?className="primary-btn"/)
  assert.match(installationPathModalSource, /className="secondary-btn"[\s\S]*?type="submit"[\s\S]*?className="primary-btn"/)
  assert.doesNotMatch(uninstallConfirmSource, /uninstall-danger-btn primary-btn|primary-btn uninstall-danger-btn/)
