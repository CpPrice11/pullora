# Roadmap Air Launcher після v2.1.1
...
Мета: користувач завжди має розуміти, наскільки свіжі й надійні дані в Library.

Scope:

- Об’єднати статуси `Оновлено`, `версії перевірено`, cache/offline/rate-limit у компактний trust/status block.
- Зробити GitHub status більш явним: online, cached, offline, rate limit, partial check.
- Додати дію `Повторити` прямо в cache/offline/rate-limit станах.
- Поліпшити empty state для filter `Оновлення`:
  - усі програми актуальні;
  - ще не перевірено;
  - частину версій не вдалося перевірити.
- Після install/update показувати короткий “щойно встановлено” або “щойно оновлено” стан на картці.
- Якщо installed store не прочитався, показувати recovery action `Повторити читання`.
- Не повертати окрему сторінку Updates у sidebar.

Done when:

- Library не виглядає “мовчазною” під час GitHub проблем.
- Cache state не сприймається як свіжі дані.
- Користувач бачить, що можна зробити далі.
- Українська й англійська версії мають усі нові рядки.
...
Recommended next implementation milestone: `v2.2.0 - Library Trust Polish`.
